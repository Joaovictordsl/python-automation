...
def executar():
    try:
        ...
        # O script aguarda você resolver o captcha e o email abrir
        input("\nApós o email abrir, aperte ENTER aqui para salvar os dados...")

        # --- PARTE ALTERADA: SALVAMENTO FORMATADO PARA O SHOPIFY ---
        with open("contas_outlook.txt", "a") as f:
            # Salvando no formato exato que o script do Shopify vai ler
            f.write(f"Email: {email} | Senha: {senha}\n")
        
        print(f"Dados salvos com sucesso em contas_outlook.txt")
